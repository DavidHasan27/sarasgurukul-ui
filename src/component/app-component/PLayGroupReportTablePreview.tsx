...
const PLayGroupReportTablePreview = ({
  data,
  student,
  onComplete,
  edit,
    ...
    if (edit) {
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        answerObj[reportQuestions.id] = {
          needImprovement: data[i].needImprovement,
          satisfactory: data[i].satisfactory,
          good: data[i].good,
          excellent: data[i].excellent,
          grade: data[i].grade,
        };
        answeridObj[reportQuestions.id] = data[i].id;
        for (let j = 0; j < data[i].child.length; j++) {
          const reportQuestionsChild = data[i].child[j].reportQuestions;
          console.log("reportQuestions2222", reportQuestionsChild);
          answerObj[reportQuestionsChild.id] = {
            needImprovement: data[i].child[j].needImprovement,
            satisfactory: data[i].child[j].satisfactory,
            good: data[i].child[j].good,
            excellent: data[i].child[j].excellent,
            grade: data[i].child[j].grade,
          };
  ...
  const onSubmit = (publish: any) => {
    ...
    for (let i = 0; i < data.length; i++) {
      for (let j = 0; j < data[i].child.length; j++) {
        if (!answer.hasOwnProperty(data[i].child[j].reportQuestions.id)) {
          errorFound = true;
          setError(true);
          break;
        } else if (answer.hasOwnProperty(data[i].child[j].reportQuestions.id)) {
          if (
            isShowError(data[i].child[j].reportQuestions.id) ||
            isShowGradeError(data[i].child[j].reportQuestions.id)
          ) {
            errorFound = true;
            setError(true);
            break;
                ...
                return (
                  <>
                    ...
                    {item.child.map((subItem: any) => {
                      ...
                      //  : ""
                      const errorObj =
                        error && isShowError(subItem.reportQuestions.id);
                      ...
                      if (
                        subItem.reportQuestions.id ===
                        "84b749e8-a89e-46a6-8503-6d993f256608"
                      ) {
                        console.log(
                          "answer",
                          subItem.reportQuestions.id,
                          answer[subItem.reportQuestions.id],
                          answer[subItem.reportQuestions.id]["grade"]
                        );
                      }

                      return (
                        <div
                          className="w-[100%] h-[50px] flex flex-row border-b border-gray-300"
                          key={subItem.id}
                        >
                ...
                <td>
                  <div>
                    <div className="w-full flex flex-row px-3">
                      ...
                      <form className=" bg-white rounded shadow-xl min-h-[400px] flex-1 border border-gray-300 pb-10 mt-2">
                            ...
                            return (
                              <>
                                ...
                                {item.child.map((subItem: any) => {
                                  ...
                                  return (
                                    <div
                                      className="w-[100%] h-[40px] flex flex-row border-b border-gray-300"
                                      key={subItem.id}
                                    >
                                      <div className="w-[28%] h-full  border-r border-gray-300 items-center  flex flex-row">
                                        <div
                                          className={`ml-4 text-gray-800 text-left  h-full flex flex-row items-center ${
                                            subItem.reportQuestions.title
                                              .length > 34
                                              ? "text-[11px] "
                                              : "text-sm"
...
export default PLayGroupReportTablePreview;
