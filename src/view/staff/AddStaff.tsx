...
const AddStaff = () => {
  ...
  const { loading, error, success, roleList, newuser } = useAppSelector(
    (state: any) => state.user
  );

  const { optionSchoolList } = useAppSelector((state: any) => state.school);

  console.log("User Created Successfully :::", newuser, success);
  ...
  return (
    <ParentLayout
      loading={loading}
      error={error}
      success={newuser ? "New user created successfully" : ""}
